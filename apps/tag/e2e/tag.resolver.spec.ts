      ...
      request(app.getHttpServer())
        ...
        .then(({ body }) => {
          const {
            ...
            TagTodoItemDTO & { todoItem: TodoItemReferenceDTO }
  ...
  describe('query', () => {
    it(`should return a connection`, () =>
      request(app.getHttpServer())
        ...
        .expect(200)
        .then(({ body }) => {
          const { edges, pageInfo }: CursorConnectionType<TagDTO> =
            body.data.tags;
          expect(pageInfo).toEqual({
    ...
    it('should require an id', () =>
      request(app.getHttpServer())
        ...
        .then(({ body }) => {
          expect(body.errors).toHaveLength(1);
          expect(body.errors[0].message).toBe(
            'Field "DeleteOneTagInput.id" of required type "ID!" was not provided.',
          );
        }));
  });
