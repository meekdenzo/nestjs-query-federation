  ...
  describe('query', () => {
    it(`should return a connection`, () =>
      request(app.getHttpServer())
        ...
        .expect(200)
        .then(({ body }) => {
          const { edges, pageInfo }: CursorConnectionType<UserDTO> =
            body.data.users;
      ...
      request(app.getHttpServer())
        ...
        .then(({ body }) => {
          ...
          expect(body.errors[0].message).toBe(
            'Field "UpdateOneUserInput.id" of required type "ID!" was not provided.',
          );
        }));
    ...
    it('should require an id', () =>
      request(app.getHttpServer())
        ...
        .then(({ body }) => {
          expect(body.errors).toHaveLength(1);
          expect(body.errors[0].message).toBe(
            'Field "DeleteOneUserInput.id" of required type "ID!" was not provided.',
          );
        }));
  });
